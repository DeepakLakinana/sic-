...
OUT_DIR = "/mnt/user-data/outputs/result_analysis"
...
def build_question_bank():
    ...
    print(f"[OK] question_bank.xlsx  ({len(df)} questions)")
    return df
...
def build_student_responses(qdf):
    ...
    """
    student_names = ["Deepak", "SDheemanth", "Preetesh",
                     "Aayush", "Santosh"][:NUM_STUDENTS]
    ...
    print(f"[OK] student_responses.xlsx  ({len(df)} students)")
    return df
...
def generate_student_pdf(result, output_dir):
    ...
    overview_data = [
        ...
        ["Correct Answers",           str(result["Correct"])],
    ...
    rec = result["Records"]
    q_data = [["Q#", "Subject", "Level", "Type", "Marks", "Max", "Time(s)", "Correct"]]
...
def main():
    ...
    print("   student_responses.xlsx")
    for res in all_results:
        sid = res["Student_ID"]
        name = res["Name"].replace(" ", "_")
        print(f"   {sid}_{name}_Report.pdf")
    print("   EXAMINER_Summary.pdf")
